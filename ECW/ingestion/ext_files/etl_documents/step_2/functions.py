...
def build_postgres_jdbc_url(secret: dict, db_name: str | None = None) -> str:
    host = secret["host"]
    port = secret.get("port", 5432)
    db = db_name or secret.get("dbname") or secret.get("database")
    if not db:
        raise ValueError("No se encontró dbname/database en el secreto y no se pasó db_name.")
...
def read_pg_query(
    ...
    *,
    ...
    """
    Lee un SELECT por JDBC y regresa Spark DataFrame.
    Importante: lo envuelve como subquery para compatibilidad.
    """
    dbtable = f"({sql}) AS q"
    return (
...
def read_parquet(spark,s3_path: str,required_columns: list | None = None) -> DataFrame:
    print(f"📂 Leyendo parquet desde: {s3_path}")

    try:
        df = spark.read.parquet(s3_path)
    except Exception as e:
        raise RuntimeError(f"❌ Error leyendo parquet en {s3_path}: {str(e)}")
