...
def get_billing_collections(conn):
    query = """
        select * from raw_bank.billing_collections;
    """
    return run_query(conn, query, "get raw_bank.billing_collections")


def get_payments_detail(conn):
    query = """
        select 
        pd.payment_id,
        pd.payment_detail_id,
        pd.facility_group_id AS posted_facility_group_id,
        pd.payment AS posted_payment
        --p.distributed_payment,
        --p.payment AS total_payment,
        --p.distribution_flag
        from public.fact_claims_payments_detail AS pd
        LEFT JOIN public.fact_payments AS p on p.payment_id=pd.payment_id
        where pd.payment_id NOT IN (1,3,4); 
    """
    return run_query(conn, query, "get payments_detail")

#---> SNS
def enviar_correo_sns(asunto_2, mensaje_time):

    asunto = asunto_2 + ' Update Billing Colecctions Step 2' 
