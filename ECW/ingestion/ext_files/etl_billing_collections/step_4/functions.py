...
def get_calendar_detail(conn):
    query = """
            SELECT
            date AS bank_dept_dt,
            calendar_week_id AS bank_dept_calendar_week_id,
            year AS bank_dept_year,
            period AS bank_dept_period,
            week AS bank_dept_week
            FROM aux.calendar_detail
            WHERE date>='2023-01-01';
        """
    return run_query(conn, query, "get_calendar_detail")


def get_billing_coll_rep(conn):
    query = """
        SELECT
            dcoll.date as bank_dept_dt,
            --c.calendar_week_id AS bank_dept_calendar_week_id,
            --cw.week AS bank_dept_week,
            --cw.period AS bank_dept_period,
            --cw.year AS bank_dept_year,
            dcoll.facility_group_id,
            --fg.facility_group_name,
            CASE WHEN b.amount IS NULL THEN 0 ELSE b.amount END AS amount ,               
            CASE WHEN dd.deduct IS NULL THEN 0 ELSE dd.deduct END AS deduct              
            --CASE WHEN t.target IS NULL THEN 0 ELSE round(t.target) END  AS daily_coll_target,
            --CASE WHEN t.target IS NULL THEN 0 ELSE round(t.target*0.86) END  AS daily_coll_target_86,
            --CASE WHEN t.target IS NULL THEN 0 ELSE round(t.target*1) END  AS daily_coll_target_100,
            --CASE WHEN t.target IS NULL THEN 0 ELSE round(t.target*1.12) END  AS daily_coll_target_112
        FROM raw_bank.dates_base_bill AS dcoll
        LEFT JOIN (SELECT
                        sheet_date,
                        SUM(amount) AS amount,
                        dist_facility_group_id
                    FROM raw_bank.bill_facility_coll  
                    GROUP BY sheet_date, dist_facility_group_id
                    ) AS b ON b.sheet_date=dcoll.date AND b.dist_facility_group_id=dcoll.facility_group_id
                    --LEFT JOIN aux.calendar_detail AS c ON c.date=dcoll.date
                    --LEFT JOIN aux.target_collections AS t ON dcoll.date=t.date AND dcoll.facility_group_id=t.facility_group_id
                    LEFT JOIN aux.bank_daily_deduct AS dd ON dd.facility_group_id=b.dist_facility_group_id
                    --LEFT JOIN aux.calendar_week AS cw ON cw.calendar_week_id=c.calendar_week_id
                    LEFT JOIN aux.facility_groups_info as fg ON fg.facility_group_id=dcoll.facility_group_id
                    order by dcoll.date,dcoll.facility_group_id
        ;"""
    return run_query(conn, query, "get_billing_coll_rep")
...
def get_facilities(conn):
    query = """
            ...
            """
    return run_query(conn, query, "get_facilities")

def get_target_coll(conn):
    query = """
            select
                date as bank_dept_dt,
                facility_group_id,
                target AS daily_coll_target
            from aux.target_collections;
        """
    return run_query(conn, query, "get_target_coll")

def get_sheet_coll_rep(conn):
    query = """
            SELECT 
                sc.*, 
                c.week AS sheet_week,
                c.period AS sheet_period,
                c.year AS sheet_year
            FROM raw_bank.bill_sheet_coll AS sc
            LEFT JOIN aux.calendar AS c ON sc.sheet_date=c.date;
        """
    return run_query(conn, query, "get_sheet_coll_rep")

#---> SNS
def enviar_correo_sns(asunto_2, mensaje_time):

    asunto = asunto_2 + ' Update Billing Colecctions Step 4' 
