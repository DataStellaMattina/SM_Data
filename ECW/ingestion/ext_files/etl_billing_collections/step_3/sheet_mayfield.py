...
def build_collections_mayfield():
    sheet_name = 'MAYFIELD'
    required = [
            "DB_SECRET_NAME",
            "AWS_REGION",
    ...
    try:
        ...
        mayfield_sheet = f.get_collections(sheet_name,conn_raw)
    ...
    facility_collections = mayfield_sheet.groupby(by=['sheet_date'])[['amount']].sum().reset_index()
    facility_collections['dist_payment'] = facility_collections['amount']
    facility_collections['dist_facility_group_id']=11#mayfield id
