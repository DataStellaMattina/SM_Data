...
#main
def get_dates_from_bill(conn):
    query ="""
            SELECT date FROM aux.calendar_detail
            WHERE calendar_week_id IN (
            SELECT DISTINCT(calendar_week_id)
            FROM aux.calendar_detail
            WHERE date >= (SELECT MIN(sheet_date) FROM raw_bank.billing_collections)
            AND date <= (SELECT MAX(sheet_date) FROM raw_bank.billing_collections));
        """
    return run_query(conn, query, "get_dates_from_bill")

def get_facilities_from_bill(conn):
    query ="""
            SELECT DISTINCT(dist_facility_group_id) AS facility_group_id
            FROM raw_bank.bill_facility_coll
            order by facility_group_id;
        """
    return run_query(conn, query, "get_facilities_from_bill")

#WSC
def get_collections(sheet_name,conn):
    query = """
        select * from raw_bank.billing_collections 
        where sheetname='{}';""".format(sheet_name)
    
    return run_query(conn, query, "get_collections")


def get_collections_det(sheet_name,conn):
    query = """select * from raw_bank.billing_collections_det 
                where sheetname='{}';""".format(sheet_name)
    
    return run_query(conn, query, "get_collections_det")

#---> SNS
def enviar_correo_sns(asunto_2, mensaje_time):

    asunto = asunto_2 + ' Update Billing Colecctions Step 3 ' 
