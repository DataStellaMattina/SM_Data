...
def build_collections_merit():
    sheet_name = '8897 - 9929 - 9879 MERIT'
    required = [
            "DB_SECRET_NAME",
            "AWS_REGION",
            "ENV"
    ...
    try:
        ...
        merit_sheet = f.get_collections(sheet_name,conn_raw)
    ...
    # --------------------- Transforms --------------------->
    print("⚙️ Iniciando transformaciones sheet merit...")
    ####### DISTRIBUTED, UNPOSTED, POSTED NODB
    facility_collections = merit_sheet.groupby(by=['sheet_date'])[['amount']].sum().reset_index()
    facility_collections['dist_payment'] = facility_collections['amount']
    facility_collections['dist_facility_group_id']=4
